...
@Component({
  ...
  template: `
    <!-- COLLAPSED TILE SLOT -->
    <div class="tile-shell" (click)="open()">
...
export class ExpandableTileComponent implements OnDestroy {
  ...
  private updateBodyExpanded(isExpanded: boolean): void {
    ...
    if (isExpanded) {
      ExpandableTileComponent.expandedCount += 1;
      if (ExpandableTileComponent.expandedCount === 1) {
        body.classList.add('tiles-expanded');
      }
      return;
    }

    ExpandableTileComponent.expandedCount = Math.max(0, ExpandableTileComponent.expandedCount - 1);
    if (ExpandableTileComponent.expandedCount === 0) {
